...
The reactors studied in this thesis belong to two families that have, at first
sight, little in common. Lead-cooled fast reactors (LFRs) are one of the
concepts of the fourth generation: liquid lead allows the plant to operate at
low pressure and at temperatures higher than those of light-water reactors, and
the fast spectrum fissions plutonium and the minor actinides, so that the same
core produces energy and reduces the long-lived waste. Part~\ref{part:lfr}
considers two of them, the ALFRED core, the European demonstrator of the
technology, in the specification of the benchmark coordinated by the Nuclear
Energy Agency~\cite{neaBench}, and the Westinghouse LFR. Heat-pipe micro
...
reactivity is governed by drums rotating in the radial reflector. The design
considered is the micro reactor developed at Argonne National Laboratory as a
modelling exercise meant to collect in one core the features that
characterize the class, heat pipes, TRISO fuel, yttrium-hydride moderator pins
and control drums~\cite{Stauff2024}, whose models are distributed on the
Virtual Test Bed~\cite{moose_vtb_mrad_description}.
...
that chain as it stands. For a fast system the constants are generated with a
Monte Carlo code on the design itself, and the choices that light-water
practice settled long ago, the environment in which the weighting flux is
computed and the mesh over which the energy is condensed, are open again. For a
...
against the angle of the drums~\cite{Roskoff2022physor}. The designs considered
here are, moreover, still on the drawing board, so that no operating experience
stands behind their models, and for both families the reference against which a
model is judged is the explicit Monte Carlo solution of the full core.

\section{Motivations}

Neutronics has, in the Monte Carlo method, a reference model. The method
follows the neutrons one by one through the actual geometry of the system with
the continuous-energy nuclear data, and its general-purpose
implementations~\cite{mcnp63,Serpent_code_description,openmc} are the standard
against which every approximate description of a core is measured; time-dependent
modes exist as well~\cite{serpentDynamic}. That the accuracy of these codes is
...
recomputed with it. The second rule is that the reference is a statistical
estimate and is treated as one. In Chapter~\ref{ch:energy-grid} the difference
between two energy structures is read against the statistical uncertainty of
the reference values, and Part~\ref{part:hpmr} builds for the interpolated
matrix an estimate of the uncertainty it inherits from the tallies, derives it
from the correlations among the tallied coefficients and validates it against
