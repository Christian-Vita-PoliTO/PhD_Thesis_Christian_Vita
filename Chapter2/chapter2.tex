...
treated as uniform. The generation of that set is a calculation in its own
right, and it has been organized for decades as a sequence of steps of
decreasing detail: for light-water reactors the established route proceeds from
the point-wise libraries to a self-shielded set at the pin level, then to a
fine-group set at the assembly level, and finally to the few-group data with
which the full core is solved~\cite{cacuci2010handbook}. Whatever the route, the
accuracy of everything computed afterwards is bounded by the quality of its
outcome, and that bound cannot be recovered by refining the solver.
...
Monte Carlo calculation against a set of mini-core models, one for each region
of the system, and weighs the accuracy each route delivers
against the computational time it costs when several configurations of the core
have to be analysed. Chapter~\ref{ch:energy-grid} concerns the energy grid on
which the condensation is performed, whose boundaries are treated as the unknown
of an optimization rather than as a convention inherited from practice. The two
works share the same scheme, in which the Monte Carlo code Serpent~2
...
constants, the assumptions under which that definition can be applied, and the
error that follows from them.
...
\end{equation}
%
the total flux, that is the angular flux integrated over the whole solid angle.
The two terms on the left describe the neutrons that leave a point of the phase
space, by streaming out of the volume element and by interacting; the two on the
right describe those that enter it, by scattering from another energy and
another direction and by fission. The multiplication factor $k_{\mathrm{eff}}$
is the eigenvalue that makes the balance stationary, and the fission source is
written as isotropic, which the emission of fission neutrons is to a very good
approximation.
...
averages over the histories, so that its error is statistical and can be reduced,
at a known cost, by simulating more of them. A deterministic code discretizes all
of them, and its error is systematic: it is decided by the discretization and it
does not disappear with computational effort. The discretization of the energy
variable is what this Part is concerned with.
...
$E_{g}<E<E_{g-1}$. Integrating Eq.~(\ref{mg:eq:transport}) over one such interval
returns an equation of identical structure, written in terms of the integrals of
the flux over the interval and of a set of constants, provided that every
constant is defined so as to preserve the corresponding rate. For a generic
reaction $x$ the definition that does so is
%
\begin{equation}
  \Sigma_{x,g}(\vec{r},\vec{\Omega}) =
  \frac{\displaystyle\int_{E_g}^{E_{g-1}}\Sigma_x(\vec{r},E)\,
        \phi(\vec{r},E,\vec{\Omega})\,\mathrm{d}E}
       {\displaystyle\int_{E_g}^{E_{g-1}}\phi(\vec{r},E,\vec{\Omega})\,\mathrm{d}E} ,
  \label{mg:eq:condensation-angle}
\end{equation}
%
and it already exhibits the first consequence of the condensation: the weight is
the angular flux, so that the exact group cross section acquires a dependence on
the direction which the continuous one does not have~\cite{SPH_book_Bell_Glasstone}.
Retaining that dependence would defeat the purpose of the operation, which is to
reduce the number of degrees of freedom of the problem, and it is therefore
removed by assuming that the energy and the angular dependences of the flux
separate. The weight then becomes a function of energy alone, and
Eq.~(\ref{mg:eq:condensation-angle}) reduces to
%
\begin{equation}
  \Sigma_{x,g}(\vec{r}) =
...
\end{equation}
%
which is the form used throughout this Part, together with the group flux
%
\begin{equation}
...
\end{equation}
%
and with the transfer between two groups, defined by the corresponding double
integral,
%
\begin{equation}
...
\end{equation}
%
The meaning of Eq.~(\ref{mg:eq:condensation}) is contained in what happens when
it is multiplied by the group flux. The product $\Sigma_{x,g}\Phi_g$ is by
...
therefore not an average cross section in the ordinary sense: it is the number
which, used with the group flux, reproduces a reaction rate, and the same
reading applies to Eq.~(\ref{mg:eq:scattering}) for the rate at which neutrons
are transferred from one group to another.
...
\cite{Serpent_homog_technique}.

\subsection{The weighting flux}

The definitions above share a structure and, with it, a difficulty. The weight is
the flux, which is the unknown of precisely the problem the constants are meant to
...
calculation of the system in one of its configurations. The group constants are
consequently exact for the state in which they were generated and approximate
everywhere else. This is not a defect of the procedure but a property of its
definition, and the practical question is not how to avoid the approximation but
how far the state of generation may differ from the state of use before the
...
grid, with an explicit treatment of the resonance self-shielding: ECCO, the cell
...
lifetime~\cite{collapsing}. \citet{altWeighting} compare, for the
time-dependent analysis of a lead fast reactor, weighting functions built from
the eigenfunctions of different formulations of the transport problem against
the standard criticality spectrum. \citet{eigenSpectra} carry the comparison
further, generating the group constants directly with the continuous-energy
Monte Carlo code SCONE~\cite{scone} from the spectra of the $k$-, $\alpha$- and
$c$-eigenvalue formulations and judging them on a time-dependent Monte Carlo
reference: for a transient that returns the system to criticality the
$c$-spectrum reproduces the reference kinetics most closely, and the difference
with respect to the criticality spectrum narrows as the group structure is
refined.
This Part does not act on the definition of the weighting function: it takes the
standard one and works on the two other degrees of freedom, the correction of the
constants that it produces and the placement of the group boundaries over which
the average is taken.

\section{Sources of error in the multi-group model}\label{mg:sec:errors}

Behind the compact form of Eqs.~(\ref{mg:eq:condensation}) to
(\ref{mg:eq:homogenization}) lies a set of decisions that the definitions
themselves do not settle, and on which the quality of the resulting library
depends:
%
\begin{itemize}
  \item the number of groups $G$, bounded from above by the cost of every
        calculation that will use the library and from below by the accuracy
        required of it;
  \item the position of the $G-1$ internal boundaries within the energy range;
  \item the partition of the core into the regions $V_i$ over which the data are
        homogenized;
  \item the state in which the weighting flux is computed, and the operating
        parameters, control rod position, burnup level and thermodynamic
        conditions, at which the library has to be regenerated;
  \item the definition of the weighting function itself, discussed in the
        previous section;
  \item the separability of the energy and angular dependences of the flux,
        under which Eq.~(\ref{mg:eq:condensation-angle}) was reduced to
        Eq.~(\ref{mg:eq:condensation}).
\end{itemize}
%
No general rule fixes these decisions for an arbitrary reactor concept, which is
why the generation of a multi-group library remains a case-dependent operation
and why it is examined here for one specific design. This Part addresses the
second of them in Chapter~\ref{ch:energy-grid}, and in Chapter~\ref{ch:sph-lfr}
it repairs, after the fact, the error that the third and the fourth introduce.

A discrepancy between a deterministic result and a Monte Carlo reference is the
sum of three contributions of different origin, and attributing it requires that
they be kept apart.
...
The second is the energy grid. Inside a group the shape of the flux is lost, and
...
Two features of a fast system give these contributions a particular weight. The
neutron spectrum extends over the whole energy range without the thermal peak
that anchors the structure of a light-water lattice, so that the placement of the
group boundaries enjoys more freedom and receives less guidance from the physics
of the problem. And the mean free path is long compared with the pitch of an
assembly, so that the flux inside a region is determined by a wide neighbourhood
and the environment in which the constants were generated matters more than it
does in a thermal system.
...
The second approach acts on the energy grid, treating the position of the group
boundaries as a decision variable. The structure is chosen from the internal
boundaries of a fine grid, which makes the problem combinatorial, and the
quantity to be optimized is the agreement of a set of integral parameters with
the Monte Carlo reference over several configurations of the control system at
once. Chapter~\ref{ch:energy-grid} formulates that search, replaces the reference
solver inside it with a neural metamodel so that the campaign can be repeated
many times, and recomputes every structure it proposes with the solver, so that
the conclusions rest on the solver and not on the metamodel.
