...
transients are actually performed. What they consume, however, is not the
continuous nuclear data. It is a condensed and homogenized version of them, the
...
and the accuracy of everything computed afterwards is bounded by their quality.
...
restored by correcting the constants themselves. Chapter~\ref{ch:energy-grid}
...
Boltzmann equation for the angular flux $\psi(\vec{r},E,\vec{\Omega})$, the
...
%
\begin{equation}
  \phi(\vec{r},E) = \int_{4\pi}\psi(\vec{r},E,\vec{\Omega})\,\mathrm{d}\vec{\Omega}
  \label{mg:eq:scalarflux}
\end{equation}
%
the scalar flux. The two terms on the left describe the neutrons that leave a
...
$E_{g}<E<E_{g-1}$. The group flux is the integral of the scalar flux over the
interval,
%
\begin{equation}
...
\section{The two approaches of this Part}
