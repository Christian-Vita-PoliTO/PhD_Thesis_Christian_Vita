...
system in full detail. Its cost per configuration is what confines it to a
limited number of reference states. % CV: riporta come esempi i software Serpent, openMC e MCNP

Deterministic solvers return the same quantities in a time smaller by orders of
magnitude, and they are therefore the tools with which parametric studies and
transients are actually performed. % CV: E' vero, ma si puo' usare anche Monte Carlo, non essere cosi perentorio
 What they consume, however, is not the
