...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.88\textwidth]{Chapter7/Figs/nested_databases.pdf}
...
\begin{figure}[htbp]
  ...
  \begin{tikzpicture}[
    font=\footnotesize,
    node distance = 3.2mm and 8mm,
    base/.style  = {rectangle, rounded corners=1pt, align=center, inner sep=3pt,
                    minimum height=7mm, text width=52mm},
    old/.style   = {base, draw=black!55, line width=.4pt},
    round/.style = {rounded corners=3.2mm, fill=ga!55},
    serp/.style  = {old, fill=serpent!55, text width=28mm},
    fm/.style    = {old, fill=nnc!28},
    grey/.style  = {old, fill=black!8},
    dec/.style   = {diamond, aspect=2.4, draw=black!55, fill=ga!45, align=center,
                    inner sep=0pt, text width=27mm, font=\scriptsize},
    arO/.style   = {-{Latex[length=1.6mm]}, draw=black!65, line width=.45pt},
    lbl/.style   = {font=\scriptsize, inner sep=1.5pt},
  ]
  \node[grey] (start) {power $P$; $T_{f,\min}$ and $\Delta T_{\mathrm{range}}$;\\
                       $\theta=\ang{0}$, flat fission source};
  ...
  \node[dec, below=of eig] (dT) {temperature field\\changed by less\\than $\epsilon$?};
  \node[dec, below=of dT] (crit) {$|\tilde k_{\mathrm{eff}}-1|<\delta$?};
  \node[grey, below=of crit] (end) {critical state: $\theta_{\mathrm{crit}}$,
                       $T_{f,i}$, $\tilde{\mathbf F}$};
  \node[serp, right=of interp] (db) {database of tallied matrices,
                       Sec.~\ref{cs:sec:database}};
  \node[old, round, right=of crit, text width=22mm] (bis) {bisection on $\theta$};
  \coordinate (rx) at ($(temp.east)+(52mm,0)$);
  ...
  \draw[arO] (db) -- (interp);
  \draw[arO] (dT.west) -- ++(-6mm,0) node[lbl, above, xshift=3mm] {no}
             |- (temp.west);
  \draw[arO] (crit) -- node[lbl, above] {no} (bis);
  \draw[arO] (bis.east) -- (bis.east -| rx) -- (rx |- temp.east) -- (temp.east);

  \begin{scope}[on background layer]
    \node[fill=ga!12, rounded corners=2mm, fit=(temp)(interp)(eig)(dT),
          inner sep=2.5mm] {};
  \end{scope}
  \end{tikzpicture}
...
\begin{figure}[htbp]
  ...
  \begin{tikzpicture}[
    ...
    round/.style = {rounded corners=3.2mm, fill=ga!55},
    serp/.style  = {old, fill=serpent!55, text width=28mm},
    serpN/.style = {new, fill=serpent!55, text width=28mm},
    fm/.style    = {old, fill=nnc!28},
    ...
    dec/.style   = {diamond, aspect=2.4, draw=black!55, fill=ga!45, align=center,
                    inner sep=0pt, text width=27mm, font=\scriptsize},
    decN/.style  = {dec, draw=newc, line width=1.1pt},
    arO/.style   = {-{Latex[length=1.6mm]}, draw=black!65, line width=.45pt},
    arN/.style   = {-{Latex[length=1.6mm]}, draw=newc, line width=.9pt},
    lbl/.style   = {font=\scriptsize, inner sep=1.5pt},
  ]
  \node[greyN] (start) {$t=0$, $BU_i=0$; power $P$;\\ $\theta=\ang{0}$, flat fission source};
  \node[old, below=of start] (temp) {fuel temperature field $T_{f,i}$ from
                       $\tilde{\mathbf F}$ and $P$, Eq.~(\ref{cs:eq:correlation})};
  ...
  \node[fm, below=of interp] (eig) {eigenproblem
                       $\tilde{\mathbf A}^{*}\tilde{\mathbf F}=\tilde k_{\mathrm{eff}}\,\tilde{\mathbf F}$};
  \node[dec, below=of eig] (dT) {temperature field\\changed by less\\than $\epsilon$?};
  \node[decN, below=of dT] (crit) {$\pm\sigma_k$ band\\contains 1 on\\two iterates?};
  ...
  \node[serp, right=of temp] (db) {database of tallied matrices,
                       Sec.~\ref{cs:sec:database}};
  ...
  \draw[arO] (db.west) -- ++(-3mm,0) |- ([yshift=2.2mm]interp.east);
  \draw[arN] (snap.west) -- ++(-3mm,0) |- ([yshift=-2.2mm]interp.east);
  \draw[arO] (dT.west) -- ++(-6mm,0) node[lbl, above, xshift=3mm] {no}
             |- (temp.west);
  \draw[arO] (crit) -- node[lbl, above] {no} (bis);
  \draw[arO] (bis.east) -- (bis.east -| rx) -- (rx |- temp.east) -- (temp.east);
  \draw[arN] (eol.west) -- ++(-12mm,0) node[lbl, above, xshift=6mm] {yes}
  ...
  \node[new, fill=white, below left=6mm and -34mm of end, text width=3mm,
        minimum height=3mm, inner sep=0pt] (lg1) {};
  \node[lbl, right=1.5mm of lg1, anchor=west] {introduced by the burnup extension};
  \node[old, fill=white, below=2.5mm of lg1, text width=3mm,
        minimum height=3mm, inner sep=0pt] (lg2) {};
  \node[lbl, right=1.5mm of lg2, anchor=west] {inherited from the search of Fig.~\ref{cs:fig:flow-search}};
  \end{tikzpicture}
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.5\textwidth]{Chapter7/Figs/u235_depletion_map.pdf}
