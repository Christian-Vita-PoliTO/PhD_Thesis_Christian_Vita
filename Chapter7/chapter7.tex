...
angle is common to the whole matrix. The linear scheme was adopted as the simplest candidate; more elaborate
...
spread, which is fixed a priori at each power. Finally, it is shared by the workflow and by the reference calculations against which the workflow is
...
as in the power scans of Sec.~\ref{cs:sec:bol}. When the statistical uncertainty of the database is propagated,
Sec.~\ref{cs:sec:ensemble}, the search stops instead when the $\pm1\sigma$ band of the last two iterates contains
...
avoids stopping on a single favourable bisection step. A termination condition completes the algorithm for the cycle analysis: if the
core is subcritical with the drums fully extracted, criticality cannot be
restored, and that condition defines the end of the cycle life. Figure~\tref{cs:fig:flow-search} draws the search; its
...
included. The search converges in twelve outer iterations and \SI{1.1}{s} on a single
core; the run comprises sixteen nominal evaluations of the matrix and seven
...
(Sec.~\ref{cs:sec:cost}); a Monte Carlo search with the same twelve iterations would require about one
...
The problem addressed here is different: burnup is to be added to a database
that already spans temperature and drum angle without multiplying its size.

The direct extension is a third axis, one matrix for every combination of
temperature level, drum angle and burnup step. With the twelve burnup steps used below, this amounts to
$8\times17\times12=1632$ criticality calculations, twelve times the existing
campaign, each requiring the composition of its step and therefore a preceding
depletion calculation. The database would grow
...
while the interpolated matrix keeps the temperature and drum dependence. Ratios far from unity can arise only from coefficients tallied with few
counts; the ratio is therefore limited to the interval $[0.5,2]$. At 66 months
the limit affects \SI{7}{\percent} of the
...
the same restart file were re-tallied at four further angles, \ang{45},
...
The effect on the critical drum angle over the cycle was also checked, by
running the cycle with the ratio interpolated between the five tallied angles
instead of taken from the single reference angle,
Fig.~\tref{cs:fig:thetacost}. The critical angle differs
...
the same composition, the same drum angle and an independent seed. The result, $\Delta k_{\mathrm{eff}}=-9\pm15$~pcm, is compatible with zero
within the statistical uncertainty. At the level of
