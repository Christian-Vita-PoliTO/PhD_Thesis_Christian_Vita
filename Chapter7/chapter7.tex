...
The same comparison explains why the search algorithm itself becomes a
subject of study when the neutronics is Monte Carlo. Each evaluation of
...
the fission matrix the evaluation is deterministic and costs milliseconds,
and the plain bisection of Sec.~\ref{cs:sec:algorithm} is sufficient.
...
Burnup has entered fission matrix methods before in two forms. In the RAPID
...
between the burned and the fresh matrix multiplies a matrix interpolated over
temperature and drum angle, so that burnup adds a factor to the database
and not an axis.
...
twelve states at which the workflow was verified. Each of them is an
...
returned by the workflow and the temperature field of the workflow, through
the multiphysics interface, with the protocol of
Sec.~\ref{cs:sec:convergence}. The drums move
...
\begin{figure}[htbp]
  ...
  calculations were run; the inset
