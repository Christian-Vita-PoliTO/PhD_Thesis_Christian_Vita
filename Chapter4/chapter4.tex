...
54 were produced by exactly one search and only five by two, a signature that places the reachable population at about three hundred
structures and the present campaign at roughly one fifth of it. That figure is
a lower bound, and Sec.~\ref{eg:sec:thousand} shows how severe a one: on a
thousand searches the same estimator returns 883. A complete census is
therefore out of reach, and no claim of that kind is made here.
...
The census itself changes considerably. The estimator applied to the hundred
searches placed the reachable population at about three hundred structures;
applied to the thousand it returns 883, and the fraction of the population
actually sampled, read from the structures seen only once, rises from
\SI{46}{\percent} to \SI{80}{\percent}. The estimate obtained from a small
sample is therefore not merely uncertain but biased low, by a factor of about
three in this case, which is the reason no census claim was made.
...
\begin{table}[htbp]
  ...
  reachable population is estimated with the bias-corrected Chao1 estimator from
  the structures produced by exactly one and by exactly two searches; the
  fraction sampled is the Good--Turing coverage. Every structure of both
  campaigns was recomputed with FRENETIC.}
  ...
  \begin{tabular}{lcc}
    ...
    Estimated reachable population          & 306   & 883 \\
    Fraction of the population sampled      & \SI{46}{\percent} & \SI{80}{\percent} \\
