...
$1.072$ and then to $1.019$ across the three. For a candidate structure $I$, the agreement with
...
\end{equation}
%
Before the scaling, the three integral fitness functions are clipped from
below at a value of ten, and the flux fitness function is replaced by its
base-ten logarithm floored at unity. The threshold is applied in the units in
which each function is expressed, and Eq.~(\ref{eg:eqn:integral}) does not
express the three in the same units. For $f_k$ and $f_{\beta}$, which are
absolute deviations in pcm, ten is a few times the statistical uncertainty of
the reference, which stays below two pcm on the multiplication factor and
below one on the delayed neutron fraction, and it marks the level below which
a difference between two structures is not treated as an improvement. For $f_{\ell}$, which is a relative deviation in per cent, the same threshold
stands for ten per cent, against a reference whose own uncertainty is about
\SI{0.05}{\percent}: it is a deviation the optimized structures never
approach, and the consequence is examined in Sec.~\ref{eg:sec:active}. The
clipping of $f_k$ and $f_{\beta}$ therefore prevents a term already close to
the precision of the reference from dominating the product, while the
threshold on $f_{\ell}$ and the logarithmic floor on the flux act as a
...
The representation adopted here is the polyploid encoding of  %CV: se non sbaglio in un paper di abrate (o di aimetta) spiega perchè siamo passati dalla rappresentazione alpha (vettore con gene binario 0 1) a gamma aggiunig brevemente questo contesto e perchè siamo passati a gamma.
\citet{prevMulti}, reproduced in full because it determines both the
...
impression, %CV: non mi piace questa frase
because the two architectures differ by four ten-thousandths in the
...
which is what a coin toss would produce. % CV: mi piace la descrizione piu colorata, ma frose poco professionale (sopratutto scritta cosi)
...
\end{tabular}
\end{table}
% CV: NOTA GENERALE: vorrei che le notazioni tipo "codice" beta_CRout o cose del genere vengano sostituite da simboli matematici e pedici per le configurazioni (ad esempio conf A,B,C o cose del genere, scegli tu cosa sembra meglio e più intuitiva)
...
\begin{figure}[htbp] %CV : togli la griglia a questo plot
  \centering
...
they are useful, so that supplying them explicitly only costs it parameters. % Forse questo paragrafo potrebbe meritare una lista(?), sull'ultimo punto siamo sicuri che abbiamo fatto tutti gli studi possbili? Puoi controllare da
...
to $0.395$ against $0.044$ decades for the lifetime.
...
displacement, the metamodel errs most. The agreement between the two is measured over the thirty pairs by
Spearman's rank correlation $r_s$. Each pair is given its rank on the two scales,
first by steepness and then by error, and $r_s$ measures how far the two
rankings agree: $+1$ if the bands with the steepest target are exactly the ones
where the metamodel errs most, $-1$ if exactly the ones where it errs least,
$0$ if the two orderings have nothing to do with each other. Ranks are used
rather than the values themselves because both quantities span decades, so
that a correlation of the values would depend on the scale chosen and be
dominated by the few extreme points. Here $r_s = +0.70$. The probability that
a correlation at least this strong arises by chance between two unrelated
quantities is $2\times10^{-5}$; in what follows that probability is quoted as
$p$, and a $p$ below one per cent is read as evidence that the association is
not accidental. Excluding the flux leaves the result unchanged, $r_s = +0.68$. The flux is worth keeping in view because it
...
established, % CV: ma no, rimani piu generico, di semplicemente dei casi che abbimo nel database di training, se dici che l'abbiamo fatto strada facendo senza itegrare anche i nuovi dati ci fa sembrare pigri... 
 89\,723 of them, after the controls described above have been
...
canonical in the following. The two ends of the window are not of the same kind. The upper end is a
genuine extreme of the data, so that a structure would have to be worse than
anything computed so far to exceed one hundred. The lower end is, for twelve
of the fifteen functions, the threshold of the controls rather than the
smallest value observed: only the three power functions, to which no control
is applied, begin at a value read from the data. Reaching unity therefore does not require a structure better than every other
computed so far, but only one better than the threshold, which for the
lifetime is the ordinary case and not the exception, as
Sec.~\ref{eg:sec:active} shows. Strictly below unity a controlled term cannot
fall, since the control raises to the threshold every value that would go
there; only the three power functions could, and that would take a structure
better than any computed so far.
...
This confirms the observation of \citet{prevMulti} on a much larger sample,
...
The optimum was produced by three of the hundred searches. The campaign of
Sec.~\ref{eg:sec:thousand}, ten times as large, measures the same probability
on a sample where it is better resolved and returns a higher value,
\SI{5.9}{\percent} per search, so that three tribes have one chance in six of
finding the best structure, ten tribes fewer than one in two, and fifty tribes
\SI{95}{\percent}. The middle panel, drawn from the hundred outcomes alone, is
the more pessimistic of the two readings.
...
test. The separation into two families raises no comparable question, since it
does not rest on the sample in the same way: as Sec.~\ref{eg:sec:families} shows,
the two families are defined by a pair of boundaries that none of the 67
structures adopts together and none omits together, so the partition is a
property of the structures themselves and not of a criterion applied to them.
...
hundredth search, at about one every two searches. Of the 67 structures obtained, 54 were produced by exactly one search and only
five by two. Four structures out of five have therefore been seen once, which
is what a sample far from complete looks like: a structure that appears once
is one the search reaches rarely, and the abundance of such structures
measures how much of the set has still to appear. Extrapolating from them
places the reachable population at a few hundred structures, several times the
number obtained here, but an extrapolation of this kind counts only what is
rare enough to have been seen once and is therefore a lower bound:
Sec.~\ref{eg:sec:thousand} shows that it grows with the campaign. A complete
census is out of reach, and no claim of that kind is made here.
...
to it crosses in one direction or the other depending on the sample. The frequencies themselves are stable; their classification into three groups
is a convenience of presentation. Sec.~\ref{eg:sec:thousand} repeats the test
on a sample four times as large, where the set of never-selected boundaries
becomes stable and the consensus skeleton less so.
...
The 67 structures do not form a single group, and the split does not have to be
looked for: it is settled by two boundaries of the fast region. No structure
among the 67 retains both the boundary at $2.231$ and the one at $0.302$~MeV,
and none omits both. The choice between the two is therefore exclusive and
unavoidable, and it partitions the set exactly, 35 structures taking $2.231$~MeV
and 32 taking $0.302$~MeV, with the searches divided 47 against 53.

That choice carries others with it, which is what makes the two groups families
rather than halves of an arbitrary cut. Every structure that adopts $0.302$~MeV
also adopts $3.679$~MeV and the thermal cut at $5.4\times10^{-7}$~MeV, while the
boundary at $1.371\times10^{-5}$~MeV accompanies $2.231$~MeV in $86\%$ of the
cases, as Table~\tref{eg:tab:families} reports. Taking the number of boundaries on
which two structures disagree as the measure of how far apart they are, two
structures of the same family differ on about five of the 29, two of different
families on about ten: the coherence extends well beyond the boundaries that
define the split, and the right panel of Fig.~\tref{eg:fig:structure}, which
places the 67 structures on a plane so as to reproduce those disagreements,
shows them falling into two groups accordingly. It is not a matter of group
count either, since the two families span overlapping ranges of $G$.
...
boundaries adapts to it. The exclusion also accounts for the barrier between the
families. Crossing it means removing one boundary and adding another, and the
intermediate structures, those carrying both or neither, are exactly the ones
that never occur: no single mutation performs the passage, which is why a search
remains within the family in which its initial population happens to fall.
...
better family follow the shape of the spectrum almost three times more closely,
and the difference in fitness follows. The same quantity weighted by the bare
flux separates the two families much less, as the last row of the table shows,
which indicates that it is the importance weighting, and not the flux alone,
that decides which of the two blocks is preferable.
...
appears in the flux fitness function and, for comparison, by the bare flux. Two
structures belonging to different families disagree on $10.4$ boundaries on
average, against the internal values reported here.}
...
\midrule
    ...
    Mean disagreement within the family [boundaries] & 4.6 & 5.5 \\
    Shape variation, importance weighted & 0.138 & 0.377 \\
    Shape variation, flux weighted & 0.212 & 0.247 \\ \bottomrule
\end{tabular}
\end{table}
...
\begin{figure}[htbp]
  ...
  67 distinct structures placed on a plane by multidimensional scaling of the
  number of boundaries on which they disagree, coloured by joint fitness, the
  circled point being the best structure. The two axes carry no meaning individually, since the method only
  arranges the points so that their separations on the plane reproduce those
  disagreements as closely as possible; here the rank correlation between
...
for the variability among the solutions. The left panel of
Fig.~\tref{eg:fig:structure} reports the same frequencies separately for the
better searches, taken as the best forty per cent, and for the remaining
ones.
...
the one that intuition suggests. Figure~\tref{eg:fig:boundaries} sets the
selection frequency against the two candidate explanations, the importance
surrounding a boundary and the variation of the flux shape across it.
A natural expectation is that boundaries should
...
rates. Neither expectation is confirmed. The selection frequency runs against
the variation of the flux shape ($r_s = -0.51$, $p = 5\times10^{-3}$), and bears
no relation to the flux surrounding the boundary ($r_s = +0.17$, $p = 0.37$).
The three boundaries of the fast region where the spectrum bends most sharply,
at $1.35$, $0.821$ and $0.498$~MeV, are never used, although $18\%$ of the flux
lies there.
...
it carries. The selection frequency does follow it ($r_s = +0.53$,
$p = 3\times10^{-3}$), and Table~\tref{eg:tab:boundaries} shows the distinction
boundary by boundary: the never-selected boundaries of the fast region have less
than $2.4\%$ of the importance around them, the boundaries of the consensus
skeleton between $1.9$ and $5.0\%$. The two readings part company precisely in
the fast region, and for a physical reason. The adjoint flux of this core varies
by less than a factor of two over the whole energy range, so the importance
follows essentially the ratio $\phi/v$: the fast groups carry a substantial part
of the flux but, at those speeds, only a small part of the importance, the ratio
between the two fractions rising from $0.10$ in the highest group to unity
around $10$~keV. The optimizer places its boundaries according to the second
quantity and not the first.
...
Eq.~(\ref{eg:eqn:distributed}). The tendency governs where the boundaries go,
and not which structure wins: among the optimized structures only part of the
objective still discriminates, as Sec.~\ref{eg:sec:active} shows.
...
\subsection{A Campaign of One Thousand Searches}\label{eg:sec:thousand}

The census left open above can be settled by measurement rather than by
extrapolation, because a search on the metamodel is cheap enough for the
campaign to be repeated at a larger size. One thousand independent searches were
run with the same operators, the same stopping rule and the same fitness, the
first hundred with the seeds of Sec.~\ref{eg:sec:howmany}, so that the sample
discussed so far is contained in this one. They produced 309 distinct
structures, and all of them were recomputed with FRENETIC, four hours of
wall-clock time. The comparison with the hundred searches is collected in
Table~\tref{eg:tab:thousand}.

The result that matters most is that nothing moves at the top. The best
structure of the thousand searches is the best structure of the hundred, the
same eighteen-group grid with the same joint fitness of $8.56\times10^{6}$, so
that the additional nine hundred searches, and the 242 additional distinct
structures they produced, contain nothing better. The optimum was reached by 59
of the thousand searches, a probability of \SI{5.9}{\percent} per search against
the \SI{3}{\percent} of the smaller sample; the estimate of Sec.~\ref{eg:sec:howmany}
was pessimistic, but the conclusion drawn from it stands, since three tribes
still have only one chance in six of finding the best structure.

The census is the part that changes most. New structures were still appearing
at the thousandth search, so this campaign does not exhaust the set either,
but they appear more rarely: 54 of the hundred searches produced a structure
that no other search produced, against 198 of the thousand, so that the
probability that one more search returns something already seen rises from
about one half to four out of five. Extrapolating from those structures, as
was done above, gives a few hundred reachable structures on the hundred
searches and about nine hundred on the thousand. The factor of three between
the two figures is the measure of what such an extrapolation is worth: it
counts only what has been seen at least once, so it can only grow as the
campaign grows, and no single value of it should be read as the size of the
set.

Three of the regularities become more pronounced. The two families are still
two, of 145 and 164 structures, and the difference in true fitness between them
widens from \SI{22}{\percent} to \SI{27}{\percent}. The observation that rarely
reached structures are the poorer ones, supported on the hundred searches by a
difference significant at the level of a few per cent, becomes unambiguous: the
198 structures produced by a single search have a median true fitness of
$1.13\times10^{7}$ against $1.03\times10^{7}$ for those produced by more than
one, a difference of one part in ten with $p=1.8\times10^{-6}$. The correlation
between the frequency with which a boundary is retained and the importance
surrounding it rises from $r_s=+0.53$ to $+0.56$, while the correlation with the
variation of the flux shape stays negative at $-0.49$; the reading of
Sec.~\ref{eg:sec:recurring} is confirmed on a sample four times as large.

One result is weakened, and in a way worth recording. The consensus skeleton was
defined as the boundaries retained by more than nine structures out of ten, and
on the 67 structures it had seven members. On the 309 it has six: one of the
seven falls to \SI{87}{\percent} and crosses the threshold, while the other six
remain between \SI{90}{\percent} and \SI{100}{\percent}. Resampling confirms
that it is the threshold and not the recurrence that is fragile, since the
membership of the skeleton is reproduced in about a third of the replicas
against seven in ten before, whereas the set of never-selected boundaries, which
was the less stable of the two on the smaller sample, is now reproduced in more
than four replicas out of five. The boundaries that recur do recur, from
\SI{87}{\percent} upwards; the division into three groups by a round threshold
is what a larger sample does not support.

The campaign also provides an independent check of the metamodel, on structures
proposed by searches it never saw. Over the 309 verified structures the median
agreement between predicted and true joint fitness is \SI{1.29}{\percent} and
the rank correlation is $0.969$, against \SI{1.4}{\percent} on the 67 structures
of Sec.~\ref{eg:sec:accuracy}. The accuracy does not degrade as the searches
explore further from the region where the training set is dense.

\begin{table}[htbp]
  \centering
  \caption[The hundred searches against the thousand]{The campaign of one
hundred searches against the campaign of one thousand, which contains it. The
probability that one more search returns a structure already obtained is
estimated as the complement of the fraction of searches that produced a
structure seen only once. Every structure of both campaigns was recomputed
with FRENETIC.}
  \label{eg:tab:thousand}
  \begin{tabular}{lcc}
    \toprule
    & 100 searches & 1000 searches \\ \midrule
    Distinct structures obtained            & 67    & 309 \\
    Best joint fitness                      & \multicolumn{2}{c}{$8.56\times10^{6}$, eighteen groups} \\
    Searches reaching the optimum           & \SI{3.0}{\percent} & \SI{5.9}{\percent} \\
    Structures produced by one search only  & 54    & 198 \\
    A further search repeats a known structure & \SI{46}{\percent} & \SI{80}{\percent} \\
    Separation between the two families     & \SI{22}{\percent} & \SI{27}{\percent} \\
    Recurrence against importance, $r_s$    & $+0.53$ & $+0.56$ \\
    Boundaries retained by more than $90\%$ & 7     & 6 \\
    \bottomrule
  \end{tabular}
\end{table}
...
\begin{figure}[htbp]
  ...
  structures obtained by the three tribes of \citet{prevMulti}, recomputed
...
against $1.06\times10^{7}$. The gap, between $1.5$ and $6.0$ times at equal group count, is largely a
question of how many searches were affordable: with the reference solver
inside the loop, three tribes were what a reasonable budget allowed, and the
same three tribes are the reason this chapter exists, since it was their
disagreement that first showed the outcome to depend on the seed. Part of it,
however, cannot be attributed to the number of searches, because the objective
is not identical: in that work the lifetime was still able to separate one
structure from another, whereas the threshold adopted here holds its three
terms constant, so that those searches were spending part of their effort on a
quantity that the canonical scale no longer rewards. The comparison itself is
unaffected, since the three structures are scored on that scale exactly as
ours are.
...
the lower end of that window, which any term already at the threshold of the
controls attains, and 100 the upper end. Six of the fifteen terms score
...
times better than the optimized structure and the neutron lifetime six
...
It is specific to the reference against which the agreement is measured, and
to the precision of that reference, which enters the objective directly through the clipping of the multiplication
factor and of the delayed neutron fraction, whose threshold of ten pcm is set
a few times above the statistical uncertainty of that reference. It
...
of the canonical window for every one of them: the three flux terms, the three lifetime terms and the delayed neutron
fraction in the withdrawn and partially inserted configurations. The multiplication factor with the rods partially inserted is saturated for 61
structures out of 67, a term being counted as saturated when its scaled value
remains below $1.05$; requiring exact equality with the minimum lowers that
count to 47. Only six terms therefore discriminate among the optimized
structures, namely the delayed neutron fraction and the multiplication factor
with the rods fully inserted, the multiplication factor with the rods
withdrawn and the three power terms. The counts refer to the 67 structures of
the hundred searches; over the 309 of Sec.~\ref{eg:sec:thousand} the terms
saturated for every structure are seven, the delayed neutron fraction with the
rods withdrawn falling to 306 cases out of 309.

The eight terms do not share a single cause. Two of them, the delayed neutron fraction in the withdrawn and partially
inserted configurations, are held at the minimum by the clipping applied
before the scaling of Eq.~(\ref{eg:eqn:scaling}), in the sense for which it
was introduced: the quantity is reproduced to within the threshold, is
clipped, and stops contributing to the ranking, so that the search is directed
towards what is still worth improving. Their deviations stay below \SI{8.9}{pcm} over the 67 structures, so they are
inside the threshold without being inside the uncertainty of the reference. The three flux terms
are held there by the logarithmic floor. The three lifetime terms are held
there by a threshold which, in the units of that function, stands for ten per
cent, while over the 67 structures the deviation of the lifetime never exceeds
seven per cent: those three terms are constant by construction, and the
lifetime takes no part in the ranking. That follows from the
units in which the objective function was inherited rather than from a
deliberate exclusion, and it is stated here because it bounds what the
optimization can be said to have achieved on that quantity. The formulation
has, however, two consequences that matter here. The first is that
...
boundary displacement. The second concerns the metamodel: the quantities on which it is least
accurate, the flux functions, whose median error is between $2.0$ and $2.3\%$
on the external validation set and reaches $3.8\%$ on the structures the
searches propose, are precisely those that carry no information in the region
where the search operates, while the terms that do decide the ranking are
predicted to better than $1\%$ on both sets. The accuracy of the metamodel is therefore concentrated where
...
no single mutation can pass and which differ by $22\%$ in true fitness. Within that organisation the structures share a skeleton of seven boundaries
retained by almost every solution, all located in the epithermal region, while
five boundaries are never selected; a campaign of one thousand searches, run
afterwards in the same way and verified in the same way, produced 309 distinct
structures without displacing the optimum and confirmed the recurrence, while
showing that the exact membership of the skeleton depends on where the
threshold is placed. The placement of the recurring boundaries
