...
scale of either quantity, is $+0.70$. The probability that a correlation at
least this strong arises by chance between two unrelated quantities is
$2\times10^{-5}$. In what follows a correlation is quoted as $\rho$ and that
probability as $p$, and a $p$ below one per cent is read as evidence that the
association is not accidental. Excluding the flux leaves the result unchanged,
$\rho = +0.68$. The flux is worth keeping in view because it
...
the variation of the flux shape ($\rho = -0.51$, $p = 5\times10^{-3}$), and bears
no relation to the flux surrounding the boundary ($\rho = +0.17$, $p = 0.37$).
...
it carries. The selection frequency does follow it ($\rho = +0.53$,
