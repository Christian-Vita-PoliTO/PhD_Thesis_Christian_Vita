...
$1.4\%$ on these structures. The two exceptions are the lifetime with the rods
withdrawn, for the reason given above, and the multiplication factor with the
rods partially inserted, at $2.1\%$: the second is the regime of reduced
accuracy characterised above, appearing here in operation rather than in
diagnosis, since the structures proposed by the searches are precisely those
whose multiplication factor lies closest to the reference, which is where the
target is steepest.
