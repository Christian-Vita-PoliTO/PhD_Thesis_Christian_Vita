...
Since this work rests on comparing a thousand independent searches, the two
...
values expressed through it, are referred to as canonical in the following. The
two ends of the window are not of the same kind. The upper end is a genuine
extreme of the data, so that a structure would have to be worse than anything
computed so far to exceed one hundred. The lower end is, for twelve of the
fifteen functions, the threshold of the controls rather than the smallest value
observed: only the three power functions, to which no control is applied, begin
at a value read from the data. Reaching unity therefore does not require a
structure better than every other computed so far, but only one better than the
...
functions could, and that would take a structure better than any computed so
far.
...
identical seeds and comparing the outcomes on the same scale. The fixed
formulation is the better of the two: it reaches a best value of
$4.33\times10^{7}$ against $4.65\times10^{7}$ and a median $3\%$ lower, on 27
distinct structures against 30, which is consistent with a stationary objective
being easier to optimize than a moving one. The fixed scale also removes the
reason for re-evaluating the whole population at every generation, and the
searches run a third faster as a result. The value of the change, however, is
not the speed but the comparability: without it, the thousand outcomes could not
...
times worse and a population of 200 improves it by $15\%$ at twice the cost per
search, while the median search reaches within $1\%$ of its own final value by
generation 35 and 87 searches out of a thousand are still improving after
...
of them require $517$~s on twelve processes, so that the cost of exploring the
design space a thousand times over is smaller than the cost of two evaluations
...
The thousand searches produced 720 distinct structures, with a number of groups
between 9 and 20. Figure~\tref{eg:fig:search} summarises their behaviour. Each
search settles quickly: measured against its own final value, the median search
is within $10\%$ of it by generation 25 and within $1\%$ by generation 35, and
87 searches out of a thousand are still improving after generation 90.
Where they settle is another matter. Three searches out of a hundred end within
$5\%$ of the best structure and the median search ends $41\%$ above it, so that
a single search returns a structure of the right order but not the best one, and
which structure it returns is not determined by the problem alone. The outcomes
...
the best of the thousand. A single search lands $40\%$ above it, and three
searches, which is what a campaign with the reference solver inside the loop can
afford, land $20\%$ above; ten searches reach $6\%$, thirty reach $3\%$ and a
hundred reach $1\%$. Read the other way, the probability of landing within $5\%$
of the best structure is $2\%$ for one search, $8\%$ for three and $96\%$ for a
hundred. The gain from repeating the search is therefore neither marginal nor
...
Searches & Median excess over the best & Within $5\%$ of the best \\ \midrule
    1    & \SI{40}{\percent} & \SI{2}{\percent} \\
    3    & \SI{20}{\percent} & \SI{8}{\percent} \\
    10   & \SI{6}{\percent}  & \SI{24}{\percent} \\
    30   & \SI{3}{\percent}  & \SI{62}{\percent} \\
    100  & \SI{1}{\percent}  & \SI{96}{\percent} \\
...
sample is sufficient for a reason worth stating because it is not obvious. The
rank correlation between the selection frequency of a boundary and the
surrounding importance is computed over the 29 internal boundaries of the fine
mesh, and that number does not grow with the number of searches: more searches
stabilise the frequency attached to each boundary, but only a finer parent mesh
would add degrees of freedom to the test. Resampling confirms that the estimate
stops moving well before the thousandth search and that further searches narrow
its confidence interval without displacing it.
...
still growing at the thousandth search. Of the 720 structures obtained, 589 were
produced by exactly one search, so that four structures out of five have been
seen once, which is what a sample far from complete looks like. A further search
returns a structure already obtained in about six cases out of ten. A complete
...
rate at which it is reached is related to its quality: the 589 structures
produced by a single search have a median true fitness $32\%$ worse than the 131
...
The 720 structures are not distributed freely over the pool of boundaries: one
choice in the fast region is taken by almost all of them, and it excludes
another. No structure among the 720 retains both the boundary at $2.231$ and the
one at $0.302$~MeV. The exclusion is complete, and it is not the consequence of
a preference for one of the two, since 13 structures retain neither: the two
boundaries are alternative, and the alternative is decided in favour of the
first in $90\%$ of the cases against $9\%$ for the second.

That choice carries others with it. Among the structures that adopt
$2.231$~MeV, $89\%$ also adopt the boundary at $4\times10^{-6}$~MeV, which
appears in none of those that adopt $0.302$~MeV; conversely, $89\%$ of the
latter adopt $3.679$~MeV and the thermal cut at $5.4\times10^{-7}$~MeV, against
$19$ and $20\%$ of the former. The two blocks are therefore coherent well beyond
...
which carries $2.2\%$ of the flux and less than $0.5\%$ of the importance, and
...
matched by a difference in quality. The 645 structures that adopt $2.231$~MeV
have a median true fitness of $6.13\times10^{7}$ against $8.24\times10^{7}$ for
the 62 that adopt $0.302$~MeV, and the best structure of the first block is
better than the best of the second by more than forty per cent. The thirteen
structures that adopt neither boundary are worse than either block by a factor
between two and three, which confirms that one of the two is needed and that the
...
quantity is $0.280$ for the block that prevails against $0.156$ for the one that
does not, so that the block following the shape of the spectrum more closely,
where the objective says it matters, is the one that loses; weighted by the bare
flux the two are indistinguishable, at $0.237$ against $0.237$. The separation
into two blocks is a property of the solutions, and the difference in quality
between them is measured here; what decides it is not identified by these data.
...
Boundary [MeV] & $2.231$~MeV block [\%] & $0.302$~MeV block [\%] \\ \midrule
    ...
    $4.000\times10^{-6}$ & 89 & 0 \\
    $3.679$ & 19 & 89 \\
    $5.400\times10^{-7}$ & 20 & 89 \\
    $9.166\times10^{-5}$ & 53 & 2 \\
\midrule
    Property & $2.231$~MeV block & $0.302$~MeV block \\ \midrule
    Distinct structures & 645 & 62 \\
    Median joint fitness & $6.131\times10^{7}$ & $8.243\times10^{7}$ \\
    Mean internal disagreement [boundaries] & 9.2 & 6.7 \\
    Shape variation, importance weighted & 0.280 & 0.156 \\
    Shape variation, flux weighted & 0.237 & 0.237 \\ \bottomrule
\end{tabular}
\end{table}

\subsection{Recurring Features of the Optimized Structures}\label{eg:sec:recurring}

Beyond the choice just described, the 720 structures display regularities which
...
in $96\%$ of the structures and the next, at $2.231$~MeV, in $90\%$, while no
other exceeds $87\%$. At the other end four boundaries are used by fewer than
one structure in ten, three of them in the fast region above $0.30$~MeV. The
optimized structures therefore agree on very little, and two of them taken at
random disagree on $9.8$ of the 29 boundaries. The left panel of
Fig.~\tref{eg:fig:structure} reports the same frequencies separately for the
better searches, taken as the best forty per cent, and for the remaining ones.

\begin{table}[htbp]
\centering
\caption[The 29 internal boundaries of the fine mesh]{The 29 internal boundaries
of the fine mesh. For each one: the fraction of the 720 distinct optimized
structures that retains it, the total importance $\phi\phi^{\ddagger}/v$ of the
two groups it separates, and whether it belongs to the best structure found.}
...
surrounding a boundary and the variation of the flux shape across it. A natural
expectation is that boundaries should be placed where the spectrum changes shape
most abruptly, since a single group cannot represent a rapidly varying flux, and
that they should follow the flux itself, since a group carrying little flux
contributes little to the reaction rates. Neither expectation is confirmed. The
selection frequency runs against the variation of the flux shape
($r_s = -0.28$) and against the flux surrounding the boundary
($r_s = -0.11$). The three boundaries of the fast region where the spectrum
bends most sharply, at $1.35$, $0.821$ and $0.498$~MeV, are used by at most one
structure in fifty, although $18\%$ of the flux lies there.
...
it carries. The selection frequency follows it, weakly but with the opposite
sign to the other two readings ($r_s = +0.22$). The two readings part company
precisely in the fast region, and for a physical reason. The adjoint flux of
this core varies by less than a factor of two over the whole energy range, so
the importance follows essentially the ratio $\phi/v$: the fast groups carry a
substantial part of the flux but, at those speeds, only a small part of the
importance, the ratio between the two fractions rising from $0.10$ in the
highest group to unity around $10$~keV. The optimizer places its boundaries
according to the second quantity and not the first.
...
variation of the importance contained in its groups gives $0.641$ for the ten
best structures against $0.832$ for random structures with the same number of
groups, so that the optimized structures distribute the importance $1.3$ times
more evenly. The same measure applied to the bare flux gives $1.382$ against
$1.222$: on that quantity the optimized structures are not more even than random
...
\begin{figure}[htbp]
  ...
  which each internal boundary is selected among the 720 distinct structures, in
...
\begin{figure}[htbp]
  ...
  720 distinct structures placed on a plane by multidimensional scaling of the
  number of boundaries on which they disagree, coloured by joint fitness, the
  circled point being the best structure. The two axes carry no meaning
  individually, since the method only arranges the points so that their
  separations on the plane reproduce those disagreements as closely as possible;
  only the relative proximity of the points is to be read.}
...
point verified by FRENETIC. The attainable fitness improves by a factor of $4.4$
between $G=8$ and the minimum, which falls at $G=15$ and coincides with the best
structure of the unconstrained campaign, and it is essentially flat over a wide
range: seven of the group counts between $G=12$ and $G=20$ lie within $10\%$ of
that minimum. Twelve groups are therefore sufficient for this core and this set
...
reaching $5.0\times10^{7}$ at $G=16$ against $4.1\times10^{7}$ at $G=15$: each
...
Seventeen constrained optimizations of this kind require five minutes on the
...
of $4.1103\times10^{7}$. Its fourteen internal boundaries are marked in the last
column of Table~\tref{eg:tab:boundaries}: it adopts the boundary at $2.231$~MeV
and therefore belongs to the block that prevails, and it uses none of the four
boundaries that the searches almost never select. It outperforms every one of
...
$6.88\times10^{7}$ against $4.22\times10^{7}$ attainable, at $G=20$
$1.34\times10^{8}$ against $4.16\times10^{7}$, and at $G=13$ $3.39\times10^{8}$
against $4.37\times10^{7}$. The gap, between $1.7$ and $7.8$ times at equal
group count, is a question of how many searches were affordable: with the
reference solver inside the loop, three tribes were what a reasonable budget
allowed, and the same three tribes are the reason this chapter exists, since it
...
nineteen groups is worse by a factor of $5.2\times10^{4}$, and one with uniform
...
$8.3\times10^{-6}$~MeV, reaches a canonical fitness of $4.06\times10^{12}$, the
median is $6.65\times10^{13}$ and the worst is $7.07\times10^{16}$.
...
optimized structures. Evaluating the fifteen scaled terms for each of the 720
...
withdrawn in 707 cases out of 720 and the flux with the rods fully inserted in
718.
...
and which no optimized structure approaches, the best reaching $1.4$. That floor
...
consequence for the metamodel is favourable and worth recording, since it was
not designed: the quantity on which the metamodel is least accurate is the
lifetime with the rods withdrawn, whose median relative error over these
structures is $32\%$, and it is least accurate there precisely because the
optimized structures reproduce that quantity to a few thousandths of a per cent,
so that a large relative error corresponds to a negligible absolute one. On the
terms that decide the ranking the metamodel is accurate to about one per cent.

\subsection{Accuracy of the Metamodel on the Proposed Structures}\label{eg:sec:accuracy}

All 720 distinct structures were recomputed with FRENETIC, which allows the
...
and the true joint fitness over the 720 structures is $2.2\%$, and the rank
correlation between the two is $0.989$. The structure that the metamodel ranks
first is the second according to the solver, which lies $1.1\%$ behind the
first, a margin below the resolution of the metamodel. What the comparison
establishes is the property the workflow actually needs: the ordering produced
by the metamodel reproduces the ordering of the solver closely enough that a
search driven by predictions arrives in the same region of the design space as a
search driven by the solver would. The fifteen best structures are listed in
Table~\tref{eg:tab:ranking}.

The disagreement on the joint fitness is a property of the objective more than
of the metamodel, and the distinction is worth making because the two are easily
confused. The joint fitness is a product, so the relative errors of the terms
that enter it compound, and how many terms enter it depends on the controls: a
structure whose fifteen quantities are each predicted to half a per cent carries
a larger error on a product of nine factors than on a product of six. The effect
can be separated from any property of the metamodel by scoring the same
structures with both sets of controls. Judged with the thresholds of
\citet{prevMulti}, under which six terms discriminate, these 720 structures show
a median disagreement of $1.2\%$; judged with the thresholds adopted here, under
which nine do, the same predictions give $2.2\%$. The accuracy on the fifteen
quantities themselves does not change and is $0.39\%$, against $0.40\%$ on the
external validation set. The ordering, which is what the search uses, improves
rather than degrades.

\begin{table}[htbp]
\centering
\caption[The fifteen best of the 720 distinct structures]{The fifteen best of
the 720 distinct structures, all of which were recomputed with FRENETIC, ranked
...
correlation between predicted and true fitness is $0.989$ and the median
disagreement is \SI{2.2}{\percent}.}
...
Fig.~\tref{eg:fig:bias}, shows that thirteen of them are predicted to better than
$1.4\%$ on these structures. The two exceptions deserve separate treatment. The
first is the multiplication factor with the rods partially inserted, at $2.1\%$:
this is the regime of reduced accuracy characterised above, appearing in
operation rather than in diagnosis, since the structures proposed by the
searches are precisely those whose multiplication factor lies closest to the
reference, which is where the target is steepest. The second is the lifetime
with the rods withdrawn, whose median relative error is $32\%$, and the figure
is misleading if read as a relative error alone. The optimized structures
reproduce that quantity to a median of $0.019\%$, so that the median error of
the metamodel on it amounts to $0.007$ percentage points on $\ell_\mathrm{eff}$,
against a statistical uncertainty of $0.048$ percentage points on the reference
itself. Four structures out of 720 have an absolute error larger than that
uncertainty: the metamodel predicts the lifetime of the withdrawn configuration
several times more precisely than the reference calculation knows it, and the
large relative error is the arithmetic of dividing a small number by a smaller
one.

The left panel shows a systematic feature of surrogate-driven optimization which
is worth recording. Of the 720 predictions, 394 underestimate the true fitness.
A search that minimises a predicted quantity is attracted both to genuinely good
structures and to structures whose quality the metamodel overestimates, and
cannot distinguish between the two. The bias is modest in magnitude and it does
not affect the conclusions, because every reported quantity comes from the
solver, but it is the reason why the verification stage becomes more rather than
less necessary as the number of searches grows.

\begin{figure}[htbp]
  \centering
  \includegraphics[width=\textwidth]{Chapter4/Figs/fig_bias.pdf}
  \caption[Predicted against true joint fitness for the 720 structures]{Left, joint fitness predicted by the metamodel against the value
  obtained from the reference solver, for each of the 720 distinct structures
...
the procedure that still calls the solver during the analysis, cost $28$
core-hours for the 720 structures.
...
This work: 1000 tribes on the metamodel, all outcomes verified & 28 \\
...
720 structures the searches went on to propose, which is the population on which
...
the solver with a rank correlation of $0.989$.

The dependence on the seed is severe, and measuring it is the first result. A
single search returns a structure whose joint fitness is typically forty per
cent above the best, and three searches, which is what a campaign with the
solver inside the loop can afford, reach twenty per cent; a hundred searches
reach one per cent and find a structure within five per cent of the best in
ninety-six cases out of a hundred. The gain from repeating the search is
...
organised. They converge to 720 distinct structures, four out of five of which
were produced by a single search, and two structures taken at random disagree on
ten of the twenty-nine available boundaries. Only one boundary is retained by
more than nine structures out of ten, so that the solutions share no common
skeleton. What they do share is a single exclusive choice in the fast region:
...
Where the boundaries go is predicted, weakly but with the right sign, by the
neutron importance with which the objective function weights the flux error, and
not by the sharpness of the spectral variation nor by the flux itself, both of
which correlate negatively with the selection frequency. The optimized
structures distribute that importance among their groups $1.3$ times more evenly
...
loop by factors between $1.7$ and $8.3$. The last comparison is the one that
