...
separates two families, whose sizes, cohesion and defining boundaries are
collected in Table~\ref{eg:tab:families} and whose composition is shown
structure by structure in Fig.~\ref{eg:fig:families}. The separation is not a
matter of group count, since the two families span overlapping ranges of $G$: it
is a coherent block of mutually exclusive choices in the fast region of the
spectrum, which the figure shows as five columns that are full in one family and
empty in the other. The pair at $2.231$ and $0.302$~MeV is the clearest, the two
never occurring together in any of the 67 structures. One family adopts the block
formed by $3.679$ and $0.302$~MeV together with the thermal cut at
$5.4\times10^{-7}$~MeV, the other adopts $2.231$~MeV together with a boundary at
$1.371\times10^{-5}$~MeV.
...
FRENETIC, and the median true fitness of the better family is $22\%$ lower than
that of the other. The origin of the difference can be traced to the way the two
families condense the spectrum. For each structure the variation of the reference
flux shape within its groups was computed and weighted by the total importance
$\phi\phi^{\ddagger}/v$ of the group, which is the weight with which the flux
fitness function of Eq.~(\ref{eg:eqn:distributed}) counts an error. Where it
matters to the objective, the structures of the better family follow the shape of
the spectrum almost three times more closely, as the last two rows of
Table~\ref{eg:tab:families} report, and the difference in fitness follows. The
same quantity weighted by the bare flux separates the two families much less,
which is the point of the comparison: it is the importance weighting, and not the
flux alone, that decides which of the two blocks is preferable.

\begin{figure}[htbp]
  \centering
  \includegraphics[width=\textwidth]{Chapter4/Figs/fig_families.pdf}
  \caption[Composition of the 67 optimized structures]{The 67 distinct optimized
  structures, one per row, grouped by family and ordered within each family so
  that similar structures sit together. A dark cell means that the structure
  retains that internal boundary of the fine mesh. The five boundaries marked
  above the matrix, and labelled in the same colour below it, are the ones whose
  presence separates the two families: they are full in one family and empty in
  the other, while the remaining boundaries are chosen almost independently of
  it.}
  \label{eg:fig:families}
\end{figure}
...
appears in the flux fitness function and, for comparison, by the bare flux. The
mean distance between a structure of one family and one of the other is $10.4$
boundaries, against the internal values reported here.}
...
\midrule
    ...
    Searches ending in the family & 53 & 47 \\
