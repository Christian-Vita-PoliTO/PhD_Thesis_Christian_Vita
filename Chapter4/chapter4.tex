...
\begin{equation}
  ...
  \times10^5, \qquad
  f_{\Lambda}^I = \left|\frac{\Lambda_\mathrm{eff}^I -
  \Lambda_\mathrm{eff}^\mathrm{MC}}{\Lambda_\mathrm{eff}^\mathrm{MC}}\right|\times10^2,
...
a difference between two structures is not treated as an improvement. For
$f_{\Lambda}$, which is a relative deviation in per cent, the same threshold
...
threshold on $f_{\Lambda}$ and the logarithmic floor on the flux act as a
...
generation time is the ordinary case and not the exception, as
...
group count, measures the number of searches and not the quality of the method:
with the reference solver inside the loop, three tribes were what a reasonable
budget allowed, and the same three tribes are the reason this chapter exists,
since it was their disagreement that first showed the outcome to depend on the
seed.
...
partially inserted configurations. The multiplication factor with the rods partially inserted is saturated for 61
...
are held there by the logarithmic floor. The three generation-time terms are
held there by a threshold which, in the units of that function, stands for ten
per cent, while over the 67 structures the deviation of the generation time
never exceeds seven per cent: those three terms are constant by construction,
and the generation time takes no part in the ranking. That follows from the
