...
\chapter{Neural Network-Assisted Optimization of the LFR Multi-Group Energy Grid Driven by Genetic Algorithms}\label{ch:energy-grid}
...
condensed energy grid. The number of groups and the placement of their
...
thermal lattices~\cite{psoThermal}, genetic algorithms for multi-group searches
...
decision variable is a subset of the internal boundaries of a fine grid, so that
...
regime dictates the role it is given. The resulting workflow, summarized in Fig.~\tref{eg:fig:flow}, is presented next,
...
\node[greyO] (fine) {fine-group homogenized\\constants (30 groups)};
...
values of the integral parameters and the spatially homogenized fine-group
...
the effective neutron lifetime. The last is the adjoint-weighted mean lifetime
that the reference calculation tallies, written here $\Lambda_\mathrm{eff}$
following the works this chapter builds on; it is not the neutron generation
time, from which it differs by the multiplication factor. The remaining two concern the
...
ten; for $f_{\Lambda}$, defined as a relative deviation multiplied by $10^{2}$,
the same ten pcm is the value $0.01$. For $f_k$ and $f_{\beta}$ the threshold
stands a few times above the statistical uncertainty of the reference, which
is below two pcm on the multiplication factor and below one pcm on the delayed
neutron fraction; for $f_{\Lambda}$ it stands below it, since the reference
lifetime carries an uncertainty of about five hundredths of a per cent, so that
a structure has to reproduce the lifetime more finely than the reference
resolves it before the control takes effect. Its purpose is to mark
...
fine structure. Of the 31 boundaries of the 30-group grid the two extremes are
...
binary gene per internal boundary of the fine grid stating whether it is
...
reproduces that organization. A shared trunk of three fully connected layers of
...
grid. The production model retains the configuration branch, which was already
...
Output & $R^2$ & Rel.\ err.\ [\%] & Output & $R^2$ & Rel.\ err.\ [\%] \\ \midrule
    ...
    $f_{\Lambda,\mathrm{out}}$ & 0.99399 & 0.860 & $f_{\Lambda,\mathrm{partial}}$ & 0.99718 & 0.849 \\
    $f_{\Lambda,\mathrm{in}}$ & 0.99385 & 0.903 & $f_{P,\mathrm{out}}$ & 0.99995 & 0.167 \\
...
explanation lies in the target function itself. Searching the training database for
...
displacement, the metamodel errs most. The right panel plots one against the
other, and the thirty points rise together over two decades of steepness and
one of error, the flattest quantity, the power, being the one the metamodel
predicts best and the steepest, the flux, the one it predicts worst. The flux is
worth keeping in view because it is the control case: its
...
threshold. Strictly below unity a controlled term cannot fall, since the control
...
the searches by no more than three per cent. Ranked with one window and with the
other, the 747 structures analysed in the following keep the same best structure
and the same fifty best, and none of them moves by more than sixteen places out
of 747, the typical structure moving by one. The range that the searches explore on their
...
thousand searches so driven converge to the same best structure, they select the
same six boundaries most often, and no boundary changes its frequency of
selection by more than four points out of a hundred; the fixed scale gives a slightly narrower spread of
...
This arrangement is deliberate and follows from the limit characterized above:
...
between 9 and 20. Figure~\tref{eg:fig:search} summarizes their behaviour, with
...
grid, which is what would add anything new to the comparison.
...
That choice carries others with it, and Fig.~\tref{eg:fig:blocks} shows which.
The two panels compare, boundary by boundary, how often each block retains it.
Six boundaries separate the blocks and are drawn in colour: besides the two that
define them, a structure that adopts $2.231$~MeV takes with it the boundaries at
four and at ninety-two electronvolts, while one that adopts $0.302$~MeV takes
the boundary at $3.679$~MeV and the thermal cut at half an electronvolt. On the
remaining twenty-three the two panels are alike, and the resemblance is the
point: outside those six boundaries the two blocks build their structures in the
same way, so that the choice made in the fast region is not one preference among
many but the one that organizes the rest.

\begin{figure}[htbp]
  \centering
  \includegraphics[width=\textwidth]{Chapter4/Figs/fig_blocks.pdf}
  \caption[The two blocks of solutions, boundary by boundary]{The two blocks of
  solutions, boundary by boundary. Above, the fraction of the structures that
  adopt the boundary at $2.231$~MeV which retains each of the 29 internal
  boundaries of the fine grid; below, the same fraction for the structures that
  adopt the boundary at $0.302$~MeV, plotted downwards. The six boundaries on
  which the two blocks differ by more than fifty points are drawn in colour and
  labelled with their energy; on the others the two blocks agree.}
  \label{eg:fig:blocks}
\end{figure}

The exclusion has a direct reading. The boundaries at $3.679$ and $2.231$~MeV
are adjacent in the fine grid, so that retaining both would subdivide a region
...
would be the natural expectation. A structure represents the spectrum well when
the reference flux keeps the same shape inside each of its groups, since a group
is described by a single number. Measuring how much the flux per unit lethargy
varies inside each group, and weighting the groups by the importance they carry,
which is the weight with which the flux fitness function of
Eq.~(\ref{eg:eqn:distributed}) counts an error, the variation inside a typical
group is of the order of a sixth of its value in both blocks, and the two cannot
be told apart on it; nor can they when the weight is the bare flux. The
...
Beyond the choice described above, the 747 structures display regularities which
...
the fine grid, how often it is retained. The distribution is broad, and no
...
therefore agree on little, and two of them taken at random disagree on
$9.7$ of the 29 boundaries.

\begin{table}[htbp]
\centering
\caption[The 29 internal boundaries of the fine grid]{The 29 internal boundaries
of the fine grid. For each one: the fraction of the 747 distinct optimized
...
readings without ambiguity. In the ten best structures the group that carries
the largest share of the importance carries $11.5\%$ of it, against $17.0\%$ in
structures drawn at random with the same number of groups: the optimized
structures avoid concentrating the importance in one group. Measured on the bare
flux the comparison is reversed, $33.1\%$ against $26.5\%$, so that what the
optimization levels out is the importance and not the flux. The criterion was
never imposed explicitly: it follows from the weighting of the flux term in
Eq.~(\ref{eg:eqn:distributed}), which discriminates among the structures in the
first generations of each search, when three quarters of the population still
differ on it.
...
\begin{figure}[htbp]
  ...
  horizontal jitter, and mark the region the campaign visited. The
...
typical outcome of one of the searches of this campaign, worse than 24 of the 57 structures the
...
uniform-lethargy structure cannot be built with 19 boundaries on this fine grid,
...
The fine grid constrains which regular structures can be constructed at all.
...
obtained by suppressing a single boundary of the fine grid. Each entry is the
...
Figure of merit & Optimized & Equal & Uniform & Finest \\
    ...
    $f_{\Lambda,\mathrm{out}}$ & 1.0 & 2.1 & 2.3 & 1.3 \\
    $f_{\Lambda,\mathrm{partial}}$ & 2.2 & 1.5 & 2.4 & 1.0 \\
    $f_{\Lambda,\mathrm{in}}$ & 1.0 & 4.3 & 3.6 & 4.6 \\
...
The third reference is the finest structure that the framework allows to be
built. The fine grid has thirty groups and the collapsing routine requires the
condensed structure to have fewer, so that keeping all thirty is not an
admissible option: the finest structure available has twenty-nine groups and is
obtained by merging two adjacent fine groups, that is by suppressing one of the
twenty-nine internal boundaries. There are twenty-nine ways of doing so, and all
of them were evaluated with the reference solver. Such a structure condenses
almost nothing, and it is therefore the natural candidate for the best result
that the multi-group model can deliver. It does not deliver it. The best of the
twenty-nine, obtained by suppressing the boundary at $8.3\times10^{-6}$~MeV,
reaches a canonical fitness of $4.20\times10^{12}$; half of them are worse than
$6.81\times10^{13}$, and the worst is four orders of magnitude beyond the best.
...
energy grid. A 29-group structure condenses almost nothing, so what remains is
...
but because its condensation error and the model error nearly cancel.

Two consequences follow. The first is practical: refining the energy grid is not
...
be a fragile function of the grid, and therefore a hard one to interpolate.
...
clipping of the three integral fitness functions, whose thresholds are fixed in
relation to the statistical uncertainty of that reference. It is specific to the
...
pool it was offered, the boundaries of the 30-group parent grid, outside which
...
metamodel to be judged on exactly the population it proposed and on the quantity the search uses, the joint fitness.
Figure~\tref{eg:fig:bias} sets the two against each other, structure by
structure. The median difference between the predicted and the true joint
fitness over the 747 structures is
$2.0\%$, while the accuracy on the fifteen quantities themselves is $0.39\%$,
the same as on the external
validation set. The points fall on both sides of the diagonal, $420$ of them
below and $327$ above, so that the metamodel is no more inclined to flatter a
structure than to penalize it. The two orderings agree where the workflow needs them to: the
three structures the metamodel places first are the three the solver places
first, and seventeen of its first twenty are among the first twenty of the
solver. The structure that the metamodel ranks first is the second
...
\begin{figure}[htbp]
  ...
  searches, on the canonical scale. Points inside the shaded region are
  structures whose fitness the metamodel underestimates, and the colour gives
...
between predicted and true joint fitness, and its median over the whole set is
\SI{2.0}{\percent}.}
...
Table~\tref{eg:tab:cost} summarizes the computational budget. Assembling the
...
its accuracy matters. Every one of those structures was recomputed with
the reference solver, so that the conclusions rest on FRENETIC and not on the
metamodel; the three structures the metamodel places first are the three the
solver places first, which is what the workflow requires of it.
...
organized. They converge to 747 distinct structures, five out of six of which
